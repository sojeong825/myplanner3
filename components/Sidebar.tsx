...
type Props = {
  ...
  onOpenSettings: () => void;
  onSignIn: () => void;
  /** 좁은 화면에서만 뜻이 있다. 넓은 화면에서는 늘 펼쳐져 있다. */
  open: boolean;
  onClose: () => void;
};
...
export default function Sidebar({
  ...
  return (
    <>
      {/* 서랍이 열려 있는 동안 뒤를 덮는다. 눌러서 닫는 길이기도 하다. */}
      {open && (
        <button
          type="button"
          aria-label="메뉴 닫기"
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex w-[248px] shrink-0 flex-col gap-6 overflow-y-auto border-r border-line bg-card px-5 py-8 transition-transform duration-200 lg:sticky lg:inset-auto lg:top-0 lg:z-auto lg:h-screen lg:w-[204px] lg:translate-x-0 ${
      ...
      >
        <button
          ...
          onClick={onClose}
          aria-label="메뉴 닫기"
          className="absolute right-3 top-3 grid size-8 place-items-center rounded-full text-ink-soft transition hover:bg-soft lg:hidden"
        >

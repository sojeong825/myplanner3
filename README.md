...
| 얼마 남지 않은 일정 (D-day) | 오른쪽 위 카드 → `components/DdayList.tsx` |
...
색이 찍히는 자리(버튼, 월간/주간 토글 활성, 달력 오늘 표시, 사이드바 시계 카드)는
...
`할 일`과 `얼마 남지 않은 일정`은 컨테이너가 흰 카드라, 안쪽 항목을 배경색이 아니라
연한 보더(`border-line`, radius 10px)로 구분한다. 완료 항목도 같은 보더를 쓰고
텍스트만 흐리게 + 취소선으로 처리한다.
...
(`task`가 null이면 추가). 삭제는 버튼을 한 번 누르면 문구가 `삭제할까요?`로 바뀌고,
한 번 더 눌러야 지워진다 — 확인 모달을 따로 띄우지 않기 위해서다.
...
- **얼마 남지 않은 일정** — `is_done = false` AND `due_date`가 있는 것만, 마감일 오름차순.
...
사이드바 순서는 프로필 사진 → 이름 + 연필 → 통계 카드 → 테마 → 날짜·시계다.
...
삭제 기능도 PRD 범위 밖이라 넣지 않았다. 필요하면 `TaskList`에 버튼을 하나 더 두고
`supabase.from('tasks').delete().eq('id', id)`를 호출하면 된다(정책은 이미 열려 있다).

...
마감일은 **비워둔 채로 연다**. 한때 오늘로 미리 채워봤지만, 오늘이 아닌 날을 잡을 때
칩을 지우는 손이 한 번 더 들어가서 되돌렸다. 마감일은 선택 항목이지 기본값이 아니다.
...
색상 팔레트의 맨 앞은 '테마 기본'이라 지금 `--accent` 색이 찍힌다. 그래서 프리셋 중
**그 색과 같아 보이는 것은 빼고 그린다**(`paletteFor`). 정확히 같은 값만 걸러내면 부족하다 —
cream(`#DDB673`)과 옐로우(`#E8C46B`)처럼 값은 다른데 눈으로는 구분되지 않는 쌍이 있다.
RGB 거리 30을 기준으로 삼은 근거는 `lib/icons.tsx` 주석의 거리표에 적어뒀다.
...
`icon`/`icon_color`(0002), `user_id`와 `settings`(0003), `memo`(0004)가 뒤에 붙었다.
새로 추가하는 컬럼은 전부 nullable로 둔다 — 그래야 기존 행을 건드리지 않고 배포할 수 있다.
